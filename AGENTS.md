...
### 執行範例
若要查看套件運行狀況，請於專案根目錄執行 `examples/` 內的範例 Server 與 Client：
1. **啟動 Server**: `uv run python examples/mainServer.py`
2. **啟動 Client**: `uv run python examples/mainClient.py`
...
### 測試
- 新功能應在 `tests/`（單元）或 `tests/integration/`（與 `examples/` 內 sample 協定連動的整合測試）補上測試。
...
- `tests/`: 單元測試（根目錄 `test_*.py`）；`tests/integration/`：與 `examples/` 連動之整合測試；`examples/`：手動示範與 sample 協定（非 pytest 收集範圍）。
...
本儲存庫為 **Python 套件 `socket_package`**：提供 socket 抽象層、client／server 進入點，以及 frame 與 protocol 相關工具，供應用程式或服務整合自訂訊息框架與連線行為。性質為**函式庫**；`tests/` 內含單元測試，`tests/integration/` 為與 `examples/` 內 sample 連動的自動化整合測試，`examples/` 為手動整合示範程式。
...
- **`tests/`**：根目錄之 `test_*.py` 為單元測試；**`tests/integration/`**：標記為 `integration`、匯入 `examples` 內 sample 協定者；**`examples/`**：手動示範腳本與 sample 模組（非測試套件的一部分）
...
- `tests/`：單元測試（根目錄 `test_*.py`）；`tests/integration/`：整合測試；`examples/`：可執行示範與 sample 協定
...
- `uv run python examples/mainServer.py`：啟動示範 server
- `uv run python examples/mainClient.py`：於另一終端機啟展示範 client
...
- **位置：** `tests/`（單元）、`tests/integration/`（與 `examples` 套件連動之整合測試）
- **命名：** `test_*.py`；測試函式名稱應描述行為與預期，例如 `test_send_messages_writes_header_and_payload_in_frame`
- **涵蓋：** 建議優先補足 frame 編解碼、routing 與 config 的單元測試；驗證 `examples/` 內 sample 路由者請放在 `tests/integration/` 並加上 `@pytest.mark.integration`
