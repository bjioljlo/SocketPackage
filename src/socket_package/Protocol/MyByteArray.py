...
class MyByteArray():
    ...
    def WriteByteArray(self, input: bytearray):
        lenth = len(input)
        formatStr:str = str(lenth) + "s"
        msg:bytearray = struct.pack("I", lenth)
    ...
    def ReadByteArray(self) -> bytearray:
        output_lenth = struct.unpack_from("I", self.__msg, self.__offset)[0]
        self.__offset += struct.calcsize("I") 
        formatStr:str = str(output_lenth) + "s"
